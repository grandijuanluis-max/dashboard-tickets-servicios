...
# Inicialización de fechas para el Reset (Formato Nativo para lógica)
...
# --- FUNCIONES DE CARGA CON NORMALIZACIÓN ---
def obtener_config():
    try:
...
def obtener_datos_tickets():
    try:
        ...
        # FORZADO DE FORMATO: Aseguramos que FE_DT entienda siempre el formato DIA/MES/AÑO
...
def guardar_seguro(df_nuevo, accion_msg):
    try:
        ...
        if not base_actual_nube.empty and len(df_nuevo) < len(base_actual_nube) and "MODIFICACION" not in accion_msg:
             st.error("🚨 BLOQUEO DE SEGURIDAD: Pérdida de registros detectada. Operación cancelada.")
             return False
        conn.update(spreadsheet=url, worksheet="BD_Dashboard_Servicios", data=df_nuevo)
        return True
    except:
        st.error("❌ Falló la comunicación con la base de datos.")
        return False
...
with st.sidebar:
    ...
    if st.button("🔄 Limpiar Fechas (D/M/A)"):
        st.session_state.f_desde = date(2020, 1, 1)
        st.session_state.f_hasta = date.today()
        st.rerun()
...
# ==========================================
# SOLAPA 2: MODIFICAR (ACTUALIZA FECHA A HOY D/M/A)
# ==========================================
elif st.session_state.menu_activo == "✏️ MODIFICAR":
    if not df_f.empty:
        sel_m = st.selectbox("Ticket a editar:", df_f.apply(lambda r: f"#{r['ID_NUM']} | {r['CLIENTES']} | {r['FE_CONSULT']}", axis=1))
        id_m = int(sel_m.split(" |")[0].replace("#",""))
        idx_f = df_actual.index[df_actual["ID_NUM"] == id_m].tolist()[0]; dm = df_actual.loc[idx_f]
        with st.form("f_mod"):
            st.warning(f"Modificando Ticket #{id_m} (Original: {dm['FE_CONSULT']})")
            c1, c2 = st.columns(2)
            ...
            if st.form_submit_button("🔥 ACTUALIZAR A FECHA DE HOY"):
                ...
                df_actual.at[idx_f, "FE_CONSULT"] = hoy.strftime("%d/%m/%Y") # D/M/A
...
# ==========================================
# SOLAPA 4: REPORTES (PDF D/M/A)
# ==========================================
elif st.session_state.menu_activo == "📊 REPORTES":
    if not df_f.empty:
        t_hs = df_f["TIEMPO_RES"].sum() / 60; st.metric("Horas Totales", f"{t_hs:,.2f} hs")
        ...
        with c2:
            ...
            pdf_a.cell(0, 8, f"PERIODO: {f_desde.strftime('%d/%m/%Y')} al {f_hasta.strftime('%d/%m/%Y')}", ln=True)
            pdf_a.ln(5); pdf_a.set_font("Arial", 'B', 14); pdf_a.cell(0, 10, "GR Consulting - Analítico", ln=True, align='C')
...
# [CONSULTAR, DASHBOARDS Y PERMISOS SE MANTIENEN IGUAL PERO CON D/M/A EN LOS SELECTORES]
    ...
    if not df_f.empty:
        ...
        with st.container(border=True):
            st.subheader(f"Ticket #{id_c}"); st.info(f"**Fecha:** {dc['FE_CONSULT']}")
            pdf = FPDF(); pdf.add_page(); pdf.set_font("Arial", 'B', 16); pdf.cell(0, 15, "Ficha Técnica", ln=True, align='C'); pdf.set_font("Arial", size=10)
            pdf.cell(40, 10, "FECHA:", 1); pdf.cell(150, 10, str(dc['FE_CONSULT']), 1, ln=True)
    ...
    with tab2: st.metric("Inversión Total", f"$ {((df_dash['TIEMPO_RES']/60)*pd.to_numeric(df_dash['VALOR_HORA'])).sum():,.2f}")
