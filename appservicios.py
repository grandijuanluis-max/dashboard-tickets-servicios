...
# Inicialización de fechas para el Reset
...
# --- FUNCIONES DE CARGA CON PROTECCIÓN CRÍTICA ---
def obtener_config():
    try:
...
def obtener_datos_tickets():
    try:
        ...
        if df.empty: 
            # Re-intento si la base parece vacía por error de red
            df = conn.read(spreadsheet=url, worksheet="BD_Dashboard_Servicios", ttl=0)
        
        if df.empty: return pd.DataFrame()

        # Normalización de encabezados (quita espacios y resuelve la Ñ)
        ...
        # Formato de Fecha Estricto D/M/A
...
def guardar_seguro(df_nuevo, accion_msg):
    try:
        ...
        if not base_nube.empty and len(df_nuevo) < len(base_nube) and "MODIFICACION" not in accion_msg:
            st.error("🚨 BLOQUEO DE SEGURIDAD: El guardado se canceló para evitar pérdida de registros.")
            return False
...
if not st.session_state.autenticado:
    ...
    with st.form("login"):
        ...
        if st.form_submit_button("INGRESAR"):
            ...
            else: st.error("❌ No se pudo validar la tabla de usuarios.")
    st.stop()
...
if df_actual.empty or "CONSULTOR" not in df_actual.columns:
    st.warning("⚠️ La base de datos está vacía o no tiene la columna 'CONSULTOR'.")
    # Si la base está vacía, creamos una estructura mínima para que no se rompa la Sidebar
    if df_actual.empty:
        df_actual = pd.DataFrame(columns=["CONSULTOR", "CLIENTES", "MODULO", "ANIO", "MES", "FE_DT", "FE_CONSULT"])
...
with st.sidebar:
    ...
    st.divider()
    # Filtros con chequeo de columna para evitar el KeyError
...
df_f = df_actual.copy()
if "FE_DT" in df_f.columns and not df_f["FE_DT"].isna().all():
...
if f_ani and "ANIO" in df_f.columns: df_f = df_f[df_f["ANIO"].isin(f_ani)]
...
# ==========================================
# SECCIONES (NUEVO / MODIFICAR / REPORTES)
# ==========================================
if st.session_state.menu_activo == "➕ NUEVO":
    # Lógica de nuevo ticket (se mantiene igual pero con D/M/A)
    ...
    if not df_f.empty and "ESTADO" in df_f.columns:
        ...
        if not df_mod.empty:
            ...
            with st.form("f_mod"):
                ...
                if st.form_submit_button("🔥 ACTUALIZAR A HOY"):
                    hoy = datetime.now()
    ...
    if not df_f.empty and "TIEMPO_RES" in df_f.columns:
        ...
        # Lógica de PDF y Excel (se mantiene igual)
