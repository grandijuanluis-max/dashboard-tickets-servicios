...
def obtener_config():
    try:
        ...
        for col in df.columns:
            # Limpieza agresiva para JUANLUIS y otros: Texto, Sin espacios, Sin ".0"
...
# ==========================================
# SECCIÓN 1: NUEVO (CARGA OPTIMIZADA)
# ==========================================
...
# ==========================================
# SECCIÓN 2: MODIFICAR (EDICIÓN COMPLETA)
# ==========================================
elif st.session_state.menu_activo == "✏️ MODIFICAR":
    pend = df_actual[df_actual["ESTADO"].str.upper() != "CERRADO"].copy()
    if not pend.empty:
        opciones_m = pend.apply(lambda r: f"#{r['ID_NUM']} | {r['CLIENTES']} | {str(r['CONSULTAS'])[:40]}...", axis=1)
        sel_m = st.selectbox("Seleccione Ticket para editar:", opciones_m)
...
# ==========================================
    ...
    if not df_f.empty:
        ...
        with st.container(border=True):
            ...
            # PDF Bonito
            pdf = FPDF(); pdf.add_page(); pdf.set_font("Arial", 'B', 16); pdf.cell(0, 15, "GR Consulting - Reporte de Servicio", ln=True, align='C')
            pdf.ln(5); pdf.set_font("Arial", size=10); pdf.cell(0, 10, f"Ticket #{id_c} | Cliente: {dc['CLIENTES']} | Fecha: {dc['FE_CONSULT']}", border=1, ln=True)
            pdf.multi_cell(0, 10, f"CONSULTA: {dc['CONSULTAS']}", border=1)
            pdf.multi_cell(0, 10, f"RESPUESTA: {dc['RESPUESTAS']}", border=1)
            pdf.ln(5); pdf.set_font("Arial", 'B', 12); pdf.cell(0, 10, f"CANTIDAD DE HORAS: {float(dc['TIEMPO_RES'])/60:.2f} hs", align='R')
            st.download_button("📥 Descargar PDF Ficha", pdf.output(dest='S').encode('latin-1'), f"Ticket_{id_c}.pdf")

# ==========================================
# SECCIÓN 4: REPORTES (TOTALES Y XLS TEXTOS AL FINAL)
# ==========================================
elif st.session_state.menu_activo == "📊 REPORTES":
    st.header("📊 Reportes Analíticos")
    if not df_f.empty:
        t_min = df_f["TIEMPO_RES"].sum(); t_hs = t_min / 60
        ...
        with col_x1:
            ...
            with pd.ExcelWriter(buf, engine='openpyxl') as w:
                ...
                df_det = df_f[cols_det].copy(); df_det["HORAS"] = (df_det["TIEMPO_RES"]/60).round(2)
                df_det["CONSULTAS"] = df_f["CONSULTAS"]; df_det["RESPUESTAS"] = df_f["RESPUESTAS"]
                df_det.to_excel(w, index=False, sheet_name='Detalle_GR')
            st.download_button("📥 Descargar Excel Detallado", buf.getvalue(), "Reporte_GR_Full.xlsx")
        with col_x2:
            pdf_r = FPDF(); pdf_r.add_page(); pdf_r.set_font("Arial", 'B', 14); pdf_r.cell(0, 10, "Resumen Consolidado de Horas", ln=True, align='C')
            pdf_r.ln(5); pdf_r.set_font("Arial", size=10)
...
# ==========================================
# SECCIÓN 5: DASHBOARDS (32 KPIs - JUANLUIS FIX)
# ==========================================
elif st.session_state.menu_activo == "📈 DASHBOARDS":
    # Verificación en vivo para evitar el error de acceso
    ...
    if not any([p1, p2, p3]):
        ...
        if p1:
            with tabs[c_tab]:
                ...
                m1, m2, m3 = st.columns(3); m1.metric("Tickets", len(df_dash)); cer = len(df_dash[df_dash["ESTADO"]=="CERRADO"]); m2.metric("Eficacia", f"{(cer/len(df_dash)*100):.1f}%" if len(df_dash)>0 else "0%"); m3.metric("Hs Totales", f"{df_dash['TIEMPO_RES'].sum()/60:,.2f}")
                st.divider(); st.write("**Tickets por Cliente**"); st.bar_chart(df_dash["CLIENTES"].value_counts())
                st.write("**Distribución por Módulo**"); st.bar_chart(df_dash["MODULO"].value_counts())
            c_tab += 1
        if p2:
            with tabs[c_tab]:
...
# --- PERMISOS (ADMIN) ---
    ...
    st.header("⚙️ Gestión de Accesos y Usuarios")
    df_ed = st.data_editor(df_config, num_rows="dynamic", hide_index=True)
    if st.button("💾 Guardar Nueva Configuración"):
        conn.update(spreadsheet=url, worksheet="Config_Consultores", data=df_ed); st.success("Permisos actualizados."); st.rerun()
