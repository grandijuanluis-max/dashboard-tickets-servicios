...
def obtener_config():
    try:
        ...
        for col in df.columns:
            # Limpieza agresiva: Texto, Sin espacios, Sin ".0" (Corrección JUANLUIS)
...
def obtener_datos_tickets():
    try:
        ...
        df.columns = [str(c).strip().upper() for c in df.columns]
        # Creamos ID_NUM para evitar KeyErrors en cálculos de IDs
...
# ==========================================
# 🔑 PANTALLA DE LOGIN (VALIDACIÓN BLINDADA)
# ==========================================
if not st.session_state.autenticado:
    st.title("🔐 Acceso GR Consulting")
    with st.form("login_form"):
        # Normalizamos la entrada para comparar contra el Excel limpio
        ...
        if st.form_submit_button("INGRESAR"):
            ...
            else: st.error("Error: No se pudo conectar con la configuración.")
    st.stop()
...
# Obtener datos del usuario actual para permisos
...
# APLICACIÓN GLOBAL DE FILTROS A LA COPIA DE TRABAJO (df_f)
df_f = df_actual.copy()
...
# ==========================================
    ...
    if not pend.empty:
        op_m = pend.apply(lambda r: f"#{r['ID_NUM']} | {r['CLIENTES']} | {str(r['CONSULTAS'])[:40]}...", axis=1)
        sel_m = st.selectbox("Ticket a editar:", op_m)
...
# ==========================================
# SECCIÓN 3: CONSULTAR (CON PDF BONITO)
# ==========================================
    ...
    if not df_f.empty:
        ...
        with st.container(border=True):
            ...
            v2.write(f"**Consultor:** {dc['CONSULTOR']}\n\n**Estado:** {dc['ESTADO']}")
            ...
            pdf.set_font("Arial", size=10); pdf.ln(5); pdf.cell(0, 10, f"Ticket #{id_c} | Cliente: {dc['CLIENTES']} | Fecha: {dc['FE_CONSULT']}", border=1, ln=True)
...
# ==========================================
# SECCIÓN 4: REPORTES (TOTALES Y XLS)
# ==========================================
    ...
    if not df_f.empty:
        ...
        with colx1:
            ...
            st.download_button("📥 Descargar Excel Detallado", buf.getvalue(), "Reporte_Soporte_GR.xlsx")
        with colx2:
...
# ==========================================
# SECCIÓN 5: DASHBOARDS (32 KPIs + JUANLUIS)
# ==========================================
